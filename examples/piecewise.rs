...
fn main() {
    ...
    // Add variables.
    let x = model
        .add_var("x", Continuous, 0.0, 0.0, 1.0, &[], &[])
        .unwrap();
    let y = model
        .add_var("y", Continuous, 0.0, 0.0, 1.0, &[], &[])
        .unwrap();
    let z = model
        .add_var("z", Continuous, 0.0, 0.0, 1.0, &[], &[])
        .unwrap();
    model.update().unwrap();

    // Add constraints.
    model
        .add_constr("c0", &x + 2.0 * &y + 3.0 * &z, Less, 4.0)
        .unwrap();
    ...
    model
        .set_pwl_obj(&x, pt_u.as_slice(), pt_f.as_slice())
        .unwrap();
    model
        .set_pwl_obj(&z, pt_u.as_slice(), pt_g.as_slice())
        .unwrap();
