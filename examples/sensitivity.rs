...
fn main() {
  ...
  assert!(model.get(attr::IsMIP).unwrap() == 1, "Model is not a MIP");
  ...
  let mut vars: Vec<_> = model.get_vars().cloned().collect();
  for (idx, (&mut v, &orig_x)) in vars.iter_mut().zip(orig_sol.iter()).enumerate() {
    let mut lb = v.get(&model, attr::LB).unwrap();
    let mut ub = v.get(&model, attr::UB).unwrap();
    let vtype = v.get(&model, attr::VType).unwrap();
    let vname = v.get(&model, attr::VarName).unwrap();

    use std::mem::transmute;
    if lb == 0.0 && ub == 1.0 &&
       (vtype == unsafe { transmute('B' as i8) } || vtype == unsafe { transmute('I' as i8) }) {
      ...
      for (idxx, (&mut vv, &orig_xx)) in vars.iter_mut().zip(orig_sol.iter()).enumerate() {
        if idx != idxx {
