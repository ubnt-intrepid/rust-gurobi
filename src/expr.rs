...
impl LinExpr {
  ...
  pub fn get_value(&self, model: &Model) -> Result<f64> {
    ...
    Ok(zip(vars, self.coeff.iter()).fold(0.0, |acc, (ind, val)| acc + ind * val) + self.offset)
  }
}
...
impl QuadExpr {
  ...
  pub fn get_value(&self, model: &Model) -> Result<f64> {
    ...
    Ok(
      zip(lind, self.lval.iter()).fold(0.0, |acc, (ind, val)| acc + ind * val)
        + multizip((qrow, qcol, self.qval.iter())).fold(0.0, |acc, (row, col, val)| acc + row * col * val)
        + self.offset,
    )
  }
}
